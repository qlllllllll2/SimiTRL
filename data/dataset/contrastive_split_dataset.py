...
def _inner_slove_data(features, temporal_mat, batch_size, max_len, simi, simi_temporal_mat, vocab=None, mask=None):
    ...
    lengths_ = torch.tensor(lengths, dtype=torch.int16)
    lengths_simi_ = torch.tensor(lengths_simi, dtype=torch.int16)
    padding_masks = padding_mask(lengths_, max_len=max_len)
    simi_masks = (torch.arange(0, max_len, device=lengths_.device).type_as(lengths_).repeat(batch_size, 1).unsqueeze(1).repeat(1, len(simi[0]), 1).lt(lengths_simi_.unsqueeze(2)))
    ...
    return X, batch_temporal_mat, padding_masks, simi_X, simi_batch_temporal_mat, simi_masks

# def padding_mask_simi(lengths, max_len=None):
...
def collate_unsuperv_contrastive_split(data, max_len=None, vocab=None, add_cls=True):
    batch_size = len(data)
    features1, features2, temporal_mat1, temporal_mat2, simi_1, simi_2, simi_temporal_mat1, simi_temporal_mat2, mask1, mask2 = zip(*data)  # list of (seq_length, feat_dim)
    X1, batch_temporal_mat1, padding_masks1, simi_X1, simi_batch_temporal_mat1, simi_masks_1 = _inner_slove_data(
        features1, temporal_mat1, batch_size, max_len, simi_1, simi_temporal_mat1, vocab, mask1)
    X2, batch_temporal_mat2, padding_masks2, simi_X2, simi_batch_temporal_mat2, simi_masks_2 = _inner_slove_data(
        features2, temporal_mat2, batch_size, max_len, simi_2, simi_temporal_mat2, vocab, mask2)
    return X1.long(), X2.long(), padding_masks1, padding_masks2, \
           batch_temporal_mat1.long(), batch_temporal_mat2.long(), simi_X1, simi_X2, simi_batch_temporal_mat1, simi_batch_temporal_mat2, simi_masks_1, simi_masks_2
