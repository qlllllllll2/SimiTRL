...
class SimilarityDataset(ClusterDataset):
    ...
    def _gen_dataset(self):
        database_dataset = DownStreamSubDataset(data_name=self.dataset, data_type='database',
        ...
        detour_dataset = DownStreamSubDataset(data_name=self.dataset, data_type='detour',
                                              data_path=self.detour_data_path,
                                              vocab=self.vocab, seq_len=self.seq_len, add_cls=self.add_cls,
                                              max_train_size=None,
                                              geo2latlon=self.geoid2latlon)
        query_dataset = DownStreamSubDataset(data_name=self.dataset,
                                             data_path=self.origin_data_path, data_type='query',
                                             vocab=self.vocab, seq_len=self.seq_len, add_cls=self.add_cls,
                                             max_train_size=None,
                                             geo2latlon=self.geoid2latlon)
