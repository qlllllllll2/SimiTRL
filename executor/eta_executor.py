...
class ETAExecutor(BertBaseExecutor):
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="Train epoch={}".format(
            ...
            X, targets, padding_masks, batch_temporal_mat, simi_X, simi_batch_temporal_mat, simi_masks = batch
            ...
            simi_X = simi_X.to(self.device)
            simi_batch_temporal_mat = simi_batch_temporal_mat.to(self.device)
            simi_masks = simi_masks.to(self.device)


            graph_dict = self.graph_dict

            predictions = self.model(x=X, padding_masks=padding_masks, batch_temporal_mat=batch_temporal_mat,
                                     simi_x=simi_X, simi_batch_temporal_mat=simi_batch_temporal_mat,
                                     simi_masks=simi_masks,
