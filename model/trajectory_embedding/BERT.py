...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, num_heads, d_model, dim_out, attn_drop=0., proj_drop=0.,
                 ...
                 temporal_bias_dim=64, use_mins_interval=False, sliding_window=True):
        super().__init__()
        ...
        self.use_sliding_window = sliding_window
    ...
    def forward(self, x, padding_masks, future_mask=True, output_attentions=False, batch_temporal_mat=None):
        ...
        if self.use_sliding_window:
            ...
            distances = torch.abs(indices.view(-1, 1) - indices.view(1, -1))  # (T, T)
            local_mask = distances > 10  # True 表示超出窗口，需要 mask

            # 扩展 local_mask 到 (1, 1, T, T) 以便广播
            local_mask = local_mask.unsqueeze(0).unsqueeze(0)  # (1, 1, T, T)

            new_mask = padding_masks & (~local_mask).to(padding_masks)  # (B, 1, T, T)
            new_mask[:, :, :, 0] = True
            padding_masks = new_mask
...
class BERTDownstream(nn.Module):
    ...
    def __init__(self, config, data_feature):
        ...
        self.fusion = nn.MultiheadAttention(self.d_model, 4, dropout=0.1)
    ...
    def forward(self, x, padding_masks, batch_temporal_mat=None,
                graph_dict=None, output_hidden_states=False, output_attentions=False, simi_x=None, simi_batch_temporal_mat=None, simi_masks=None):
        """
        Args:
            x: (batch_size, seq_length, feat_dim) torch tensor of masked features (input)
            padding_masks: (batch_size, seq_length) boolean tensor, 1 means keep vector at this position, 0 means padding
        ...
        bs, num_simi, length, _ = simi_x.shape

        simi_x = simi_x.flatten(0, 1)
        simi_masks = simi_masks.flatten(0, 1)
        simi_batch_temporal_mat = simi_batch_temporal_mat.flatten(0, 1)
        simi_out, _, _ = self.bert(x=simi_x, padding_masks=simi_masks,
                                         batch_temporal_mat=simi_batch_temporal_mat, graph_dict=graph_dict,
                                         output_hidden_states=output_hidden_states,
                                         output_attentions=output_attentions)
        if self.pooling == 'cls' or self.pooling == 'cls_before_pooler':
            simi_out = simi_out[:, 0, :].view(bs, num_simi, -1)
            simi_out_feat1 = self.fusion(token_emb[:, 0, :].unsqueeze(0), simi_out.permute(1, 0, 2), simi_out.permute(1, 0, 2))[0]
            if self.add_cls:
                return simi_out_feat1[0] + token_emb[:, 0, :]  # (batch_size, feat_dim)
            else:
...
class LinearETA(nn.Module):
    ...
    def forward(self, x, padding_masks, batch_temporal_mat=None,
        ...
        traj_emb = self.model(x=x, padding_masks=padding_masks,
                              ...
                              output_hidden_states=output_hidden_states,
                              output_attentions=output_attentions,
                              simi_x=simi_x, simi_batch_temporal_mat=simi_batch_temporal_mat, simi_masks=simi_masks)  # (B, d_model)
...
class LinearSim(nn.Module):
    ...
    def forward(self, x, padding_masks, batch_temporal_mat=None,
        ...
        traj_emb = self.model(x=x, padding_masks=padding_masks,
                              batch_temporal_mat=batch_temporal_mat, graph_dict=graph_dict,
                              output_hidden_states=output_hidden_states,
                              output_attentions=output_attentions, simi_x=simi_x, simi_batch_temporal_mat=simi_batch_temporal_mat, simi_masks=simi_masks)  # (B, d_model)
...
class BERTContrastiveLM(nn.Module):
    ...
    # simi_contra_view1 = simi_contra_view1,
    # simi_contra_view2 = simi_contra_view2,
    # simi_padding_masks1 = simi_padding_masks1,
    # simi_padding_masks2 = simi_padding_masks2,
    # simi_batch_temporal_mat1 = simi_batch_temporal_mat1,
    # simi_batch_temporal_mat2 = simi_batch_temporal_mat2,
    def forward(self, contra_view1, contra_view2, argument_methods1,
                ...
                simi_contra_view1=None, simi_contra_view2=None,simi_padding_masks1=None,
                simi_padding_masks2=None,simi_batch_temporal_mat1=None,simi_batch_temporal_mat2=None,
                graph_dict=None, output_hidden_states=False, output_attentions=False):
        """
        Args:
        ...
        bs, num_simi, length, _ = simi_contra_view1.shape

        
        simi_contra_view1 = simi_contra_view1.flatten(0, 1)
        simi_padding_masks1 = simi_padding_masks1.flatten(0, 1)
        simi_batch_temporal_mat1 = simi_batch_temporal_mat1.flatten(0, 1)
        simi_out_view1, _, _ = self.bert(x=simi_contra_view1, padding_masks=simi_padding_masks1,
                                          batch_temporal_mat=simi_batch_temporal_mat1, graph_dict=graph_dict,
                                          output_hidden_states=output_hidden_states,
                                          output_attentions=output_attentions)
        simi_pool_out_view1 = self.pooler(bert_output=simi_out_view1, padding_masks=simi_padding_masks1,
                                           hidden_states=hidden_states1)
        simi_pool_out_view1 = simi_pool_out_view1.view(bs, num_simi, -1)
        simi_out_feat1 = self.fusion(pool_out_view1.unsqueeze(0), simi_pool_out_view1.permute(1, 0, 2), simi_pool_out_view1.permute(1, 0, 2))[0]
        pool_out_view1 = pool_out_view1 + simi_out_feat1.squeeze(0)
        ...
        simi_contra_view2 = simi_contra_view2.flatten(0, 1)
        simi_padding_masks2 = simi_padding_masks2.flatten(0, 1)
        simi_batch_temporal_mat2 = simi_batch_temporal_mat2.flatten(0, 1)
        simi_out_view2, _, _ = self.bert(x=simi_contra_view2, padding_masks=simi_padding_masks2,
                                         batch_temporal_mat=simi_batch_temporal_mat2, graph_dict=graph_dict,
                                         output_hidden_states=output_hidden_states,
                                         output_attentions=output_attentions)
        simi_pool_out_view2 = self.pooler(bert_output=simi_out_view2, padding_masks=simi_padding_masks2,
                                          hidden_states=hidden_states2)
        simi_pool_out_view2 = simi_pool_out_view2.view(bs, num_simi, -1)
        simi_out_feat2 = self.fusion(pool_out_view2.unsqueeze(0), simi_pool_out_view2.permute(1, 0, 2),
                                     simi_pool_out_view2.permute(1, 0, 2))[0]
        pool_out_view2 = pool_out_view2 + simi_out_feat2.squeeze(0)
